...
void ofxNCoreVision::loadXMLSettings()
{
	...
	//--------------------------------------------------------------
	winWidth					= XML.getValue("CONFIG:WINDOW:WIDTH", 1200);
...
void ofxNCoreVision::_update(ofEventArgs &e)
{
	...
	else//else process camera frame
	{
		...
		else
		{
			...
			if(contourFinder.bTrackFiducials)
			{	
				filter->applyCPUFiltersFiducial( processedImg_fiducial );
...
void ofxNCoreVision::getPixels()
{
	...
	if(PS3!=NULL)
	{
		...
		processedImg_fiducial.setFromPixels(PS3->getPixels(), camWidth, camHeight);
	}
	else if(ffmv != NULL)
	{
...
void ofxNCoreVision::grabFrameToCPU()
{
	...
	if (bcamera)
	{
            ...
            sourceImg.setFromPixels(vidGrabber->getPixels(), camWidth, camHeight);
 			//convert to grayscale
 			processedImg = sourceImg;
			processedImg_fiducial = sourceImg;
 		#endif
	}
	else
	{
...
//	ofDrawBitmapString(" www.nuigroup.com/ccv - 1.3 ", 725, 596);
	...
	if (bcamera)
	{
		...
		ofSetColor(0xFFFFFF);
		verdana.drawString(str + str2 + str4, CONTROLX, 480);
	}
	else
	{
	...
	if (bTUIOMode)
	{
		...
		verdana.drawString(buf, CONTROLX, 535);
	}
	ofSetColor(0xFF0000);
	verdana.drawString("Press spacebar for mini mode", CONTROLX, 572);
}
