...
class ProcessFilters : public Filters {
  ...
  public:
    ...
    void allocate( int w, int h ) {
		...
		fiducialImg.allocate(camWidth, camHeight); //Fiducial Image
 ...
 ****************************************************************/
	 ...
	 void applyCPUFiltersFiducial(CPUImageFilter& img){
		...
		//cvAdaptiveThreshold(img.getCvImage(),img.getCvImage(),highpassBlurFiducial,);
		//cvEqualizeHist(img.getCvImage(),img.getCvImage() );
		//img.flagImageChanged();
		

		//From Substracted Image
		if(smoothFiducial != 0)
		{
			img.blur((smooth * 2) + 1);
		}

		if( (highpassBlurFiducial != 0 ) || (highpassNoiseFiducial != 0) )
		{
			img.highpass(highpassBlurFiducial, highpassNoiseFiducial);
		}

		if( highpassAmpFiducial != 0)
		{
			img.amplify(img, highpassAmpFiducial);
		}

		
	//	cvCanny(img.getCvImage(),img.getCvImage(), 49*100, 49*1000,7 );
        img.threshold(threshold); //Threshold

		fiducialImg = img; //for drawing
    }
 ...
 ****************************************************************/
    void draw()
    {
		...
		fiducialImg.draw(685, 392, 128, 96);
    }
